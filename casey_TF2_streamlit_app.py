...
def main():
    ...
    if stock_symbol:
        try:
            ...
            # produces a (151, 100, 1) array
            x_pred = create_dataset(scaled_data)
            # print(f"x_pred dims BEFORE changing to 3D => {x_pred.shape}\n")
            # print(f"x_pred BEFORE changing to 3D => {x_pred}\n")

            # For LSTM need to make this 151, 1, 100
            x_pred = x_pred.reshape((x_pred.shape[0], 1, x_pred.shape[1]))
            # print(f"x_pred dims AFTER 3D => {x_pred.shape}, x_pred is a {type(x_pred)}\n")
            ...
            # print(f"(AFTER transform) last_100_days_scaled={last_100_days_scaled.shape}\n")
